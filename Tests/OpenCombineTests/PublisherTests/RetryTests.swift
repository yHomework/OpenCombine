...
final class RetryTests: XCTestCase {
    ...
    func testRetry3Times() {
        let subscription = CustomSubscription()
        let publisher = CustomPublisher(subscription: subscription)
        let retry = publisher.retry(3)
        let tracking = TrackingSubscriber(receiveSubscription: { $0.request(.max(5)) })
    ...
    func testRetryForever() {
        let subscription = CustomSubscription()
        let publisher = CustomPublisher(subscription: subscription)
        let retry = Publishers.Retry(upstream: publisher, retries: nil)
    ...
    func testSubscriptionRecursion() {
        ...
        var upstreamSubscribeCounter = 0
        publisher.didSubscribe = { _ in
            if 0 < upstreamSubscribeCounter && upstreamSubscribeCounter < 10 {
                publisher.send(completion: .failure(.oops))
            }
            upstreamSubscribeCounter += 1
        }
